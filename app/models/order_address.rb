class OrderAddress
  ...
  attr_accessor(:post_code, :prefecture_id, :municipality, :street_address, :building, :tell, :user_id, :product_id, :token,
                :price)
  ...
  def save
    ...
    Address.create(post_code:, prefecture_id:, municipality:,
                   street_address:, building:, tell:, order_id: order.id)
  end
